	...
	for _, ent in ipairs(DoorCache) do
		if IsValid(ent) then
			...
			if ent:DoorIsOwnable() then
        		--AddText({color_white, "Свободная дверь (F2)"})
			...
			elseif not ent:DoorIsOwnable() then
				...
				if (ent:DoorGetCoOwners() ~= nil) then
					for k, co in ipairs(ent:DoorGetCoOwners()) do
						...
						if (k >= 4) then
							AddText({color_white, 'и ' .. (#ent:DoorGetCoOwners() - 4) .. ' совладельца.'}) -- TODO: FIX
							break
						end
					end
				end
			...
			if (tr.Entity == ent) and (lw:DistToSqr(tr.HitPos) < 65) then
				cam_Start3D2D(tr.HitPos + tr.HitNormal, tr.HitNormal:Angle() + off_ang, .1)
				local h = 0
				...
				for k, v in ipairs(DoorText) do
					...
					v[1].a = a
					color_black.a = a
					local _, th = draw.ShadowSimpleText(v[2], 'rp.ui.30', 0, h, v[1], 1, 1)
					h = h + 40
				end
