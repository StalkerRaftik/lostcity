...
-- 		for _, doortable in pairs(rp.cfg.Doors) do
--       for _, doors in pairs(doortable) do
--         if table.HasValue(doors, ent:doorIndex()) then
--           for id, door in pairs(doors) do
--             table.insert(dtable,door)
--           end
--         end
--       end
--     end
--     if not pl:CanAfford(cost*table.Count(dtable)) then
--       rp.Notify(pl, NOTIFY_ERROR, rp.Term('CannotAfford'))
--       return
--     end
--     pl:TakeMoney(cost*table.Count(dtable))
--     for k, v in pairs(dtable) do
--       doorIndexToEnt(v):DoorOwn(pl)
--     end
--     rp.Notify(pl, NOTIFY_GREEN, rp.Term('DoorBought'), rp.FormatMoney(cost*table.Count(dtable)))
--     return
-- 	end
-- end)
...
-- rp.AddCommand('/selldoor', function(pl, text, args)
--   local dtable = {}
--   local cost = rp.cfg.DoorCostMin*0.7
-- 	local ent = pl:GetEyeTrace().Entity
--   if IsValid(ent) and ent:IsDoor() and ent:DoorOwnedBy(pl) and (ent:GetPos():DistToSqr(pl:GetPos()) < 13225) then
--     for _, doortable in pairs(rp.cfg.Doors) do
--       for _, doors in pairs(doortable) do
...
--       end
--     end
--     pl:AddMoney(cost*table.Count(dtable))
--     for k, v in pairs(dtable) do
--       doorIndexToEnt(v):DoorUnOwn(pl)
--     end
--     rp.Notify(pl, NOTIFY_GREEN, rp.Term('DoorSold'), rp.FormatMoney(cost*table.Count(dtable)))
--     return
--   end
-- end)
