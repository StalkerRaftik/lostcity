...
function PLAYER:ChangeTeam(t, force)
	...
	if self.sg_jail and not force then
		self:Notify(NOTIFY_ERROR, rp.Term('CannotChangeJob'), 'Вы наказаны')
		return false
	end

  -- if IsValid(self:isHandcuffed()) and not force then
  --   self:Notify(NOTIFY_ERROR, rp.Term('CannotChangeJob'), 'Вы в наручниках')
  --   return false
  -- end
  ...
  if not force then
    if type(TEAM.NeedToChangeFrom) == "number" and prevTeam ~= TEAM.NeedToChangeFrom then
        DarkRP.notify(self,1,4, "Вам нужно быть "..rp.teams[TEAM.NeedToChangeFrom].name..", чтобы стать "..TEAM.name)
        return false
    elseif type(TEAM.NeedToChangeFrom) == "table" and not table.HasValue(TEAM.NeedToChangeFrom, prevTeam) then
        ...
        DarkRP.notify(self,1,4, "Вам нужно быть "..string.sub(teamnames, 5)..", чтобы стать "..TEAM.name)
        return false
    end
    local max = TEAM.max
    ...
    max < 1 and (numPlayers + 1) / player.GetCount() > max) then -- fractional limit (in percentages)
        DarkRP.notify(self,1,4, "Достигнут лимит: "..TEAM.name)
        return false
    end
  end
  ...
  if TEAM.lvl then
    if self:GetLevel() < TEAM.lvl then
      rp.Notify(self, NOTIFY_ERROR, "Для данной провессии необходим: "..TEAM.lvl.." ур.")
      return 
    end
  end

  -- Должен быть рядом с нпц, епта. Иначе хуй соси с командой
  local checker = false
