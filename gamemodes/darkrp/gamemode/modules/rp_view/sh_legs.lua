...
if (CLIENT) then
    ...
    function ShouldDrawLegs()
        ...
        if LocalPlayer():GetNVar("PlayerTPV") == true then return false end
    ...
    local LegsThinkTimer = 0
    function Legs:Think(maxseqgroundspeed)
        if not (CurTime() > LegsThinkTimer) then return end
        LegsThinkTimer = CurTime() + 0.033
    ...
    local UpdateAnimationTimer = 0
    hook.Add("UpdateAnimation", "GML:UpdateAnimation", function(ply, velocity, maxseqgroundspeed)
        if not (CurTime() > UpdateAnimationTimer) then return end
        UpdateAnimationTimer = CurTime() + 0.033
	...
	hook.Add("RenderScreenspaceEffects", "GML:Render::Vehicle", function()
        if not (CurTime() > VehicleTimer) then return end
        VehicleTimer = CurTime() + 0.033
