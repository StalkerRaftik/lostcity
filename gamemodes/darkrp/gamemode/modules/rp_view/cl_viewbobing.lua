...
hook.Add( "CalcView", "HeadbobTest", function( pl, pos, ang, fov )
	if pl:GetMoveType() != MOVETYPE_NOCLIP and ( not ( pl:GetActiveWeapon().IsTFAWeapon ) ) then
		local v = {}
		v.pos = pos
		v.ang = ang
		v.fov = fov
		...
		local info = { speed = 1, yaw = 0.5 * ( 1 + 4 * 0), roll = 0.1 * ( 1 + 40 * 0 ) };
