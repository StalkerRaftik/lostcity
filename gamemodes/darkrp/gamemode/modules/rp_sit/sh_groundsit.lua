...
if SERVER then
	concommand.Add(tag, function(ply)
		if not ply.LastSit or ply.LastSit < CurTime() then
			...
			ply.NeedToResetSequences = ply:GetNWBool(tag)
...
hook.Add("CalcMainActivity", tag, function(ply, vel)
	...
	elseif ply:GetNWBool(tag) == false and ply.NeedToResetSequences == true then
		ply.NeedToResetSequences = false
		return 0, 0
	else
		return
	end
