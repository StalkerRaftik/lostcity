...
function ENT:Initialize()
	...
	self:SetDTInt(1, 0);
	self:SetDTInt(2, FS_Cabbage_Seed_Time);
...
function ENT:SpawnFunction(ply, trace)
	local ent = ents.Create("fs_cabbage_seeds");
	...
	if (!self.nextSec or CurTime() >= self.nextSec) then	
		if (self:GetDTInt(1) == 1) then
			...
			elseif (self:GetDTInt(2) == 0) then
				self:EmitSound("physics/surfaces/sand_impact_bullet4.wav");
				self:SetModel("models/props/de_inferno/fountain_bowl_p10.mdl");
				self:PhysicsInit(SOLID_VPHYSICS);
				self:SetMoveType(MOVETYPE_VPHYSICS);
				self:SetSolid(SOLID_VPHYSICS);
				...
				self:GetPhysicsObject():EnableMotion(false);
				self:SetDTInt(2, FS_Cabbage_Harvest_Time);
				self:SetDTInt(1, 2);
				self:GetPhysicsObject():SetMass(105);
				self:SetColor(FS_Cabbage_Color);
			end;
		end;
		if (self:GetDTInt(1) == 2) then
			if self:GetDTInt(2)>0 then
				self:SetDTInt(2, self:GetDTInt(2)-1);
			elseif (self:GetDTInt(2) == 0) then
				for i=1, FS_Cabbage_Harvest_Amount do
											
					local SpawnedFood = ents.Create("spawned_food")
					SpawnedFood.ShareGravgun = true
					SpawnedFood.onlyremover = true
					SpawnedFood:SetAngles(Angle(0,math.random(0, 360),0));
					SpawnedFood:SetPos(self:GetPos()+(self:GetUp()*math.random(0, 5))+(self:GetRight()*math.random(-2.5, 2.5))+(self:GetForward()*math.random(-2.5, 2.5)));
          			SpawnedFood:SetFoodAmount(30)
          			SpawnedFood:SetThirstAmount(30)
					SpawnedFood:SetModel("models/foodnhouseholditems/cabbage1.mdl")
					SpawnedFood.FoodName = 'Перец'
