...
if ( CLIENT ) then
	...
	function SWEP:DrawHUD()
		...
		draw.SimpleTextOutlined( "Vous êtes zippé!", "handcuffText", x, y-10, color_white, 1, 1, 1, color_black )
		draw.SimpleText( "Vous pouvez être libéré par les autres autour de vous, mais pour l'instant vous êtes coincé!", "DermaDefault", x, y + 25, color_white, 1, 1 )
		
		local cnt = self:GetNWInt("break_amount")
		
		if ( cnt > 0 ) then
			draw.SimpleText( "État de la version: "..cnt.."/"..self.BreakAmount, "DermaDefault", x, y - 35, Color( 220, 20, 20 ), 1, 1 )
		end
	end
end
...
hook.Add( "SetupMove", "Cuffs Move Penalty_ZipTie", function( ... ) moveHook( ... ) end  )
