...
self.Owner:SetNWInt("checkfilter", CurTime() + 5)
	...
	timer.Simple(self:SequenceDuration(), function()
		...
		print(self.Owner)
		if !((type == INV_HATS or type == INV_CLOTHES) && self.Owner:HaveItem(type, class)) then return end

	 	if self.Owner.ProgressBar then
			DarkRP.notify(self.Owner, 1, 4, "Вы не можете сделать это сейчас!")
			return false
		end
		...
		if type == INV_HATS then
			...
			if self.Owner.Cosmetics[data.slot] then
				DarkRP.notify(self.Owner, 1, 8, "У вас уже есть предмет в этом слоте ("..Cosmetics.Slots[data.slot].."). Сначала снимите его")
				return false
			end
				...
				self.Owner:RemoveItem(INV_HATS, class)
				self.Owner:EquipCosmetic(class)

				net.Start("UpdateClientCosmetics")
				net.Send(self.Owner)

				if self.Owner:HasGasmask() and not self.Owner:GetNWInt("GasmaskHealth") then self.Owner:SetNWInt("GasmaskHealth", 100 ) end

				hook.Run( "PlayerUseInventory", self.Owner, type, class )
