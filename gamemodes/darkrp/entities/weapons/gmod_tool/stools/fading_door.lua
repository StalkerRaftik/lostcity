...
local function checkTrace(tr)
  ...
  return tr.Entity
    ...
    and not (
      tr.Entity:IsPlayer()
      or tr.Entity:IsNPC()
      or tr.Entity:GetClass() == "ent_textscreen"
      or tr.Entity:GetClass() == "modulus_skateboard"
      or IsValid(tr.Entity.ItemOwner)
...
local function fadeDeactivate(self)
  ...
  local can, reason = hook.Run( 'AntiFadingDoorCrash', self, self:CPPIGetOwner() )
...
local function dooEet(ply, ent, stuff)
  if ent.isFadingDoor then
    ...
    ent:SetNVar('isFadingDoor', true, NETWORK_PROTOCOL_PUBLIC)
...
local function doUndo(undoData, ent)
  if IsValid(ent) then
    ...
    ent:SetNVar('isFadingDoor', false, NETWORK_PROTOCOL_PUBLIC)
...
function TOOL:LeftClick(tr)
  if not checkTrace(tr) then
    DarkRP.notify(self:GetOwner(), 1, 4, "Ты не можешь использовать FadingDoor на этом!")
    return false
  end
