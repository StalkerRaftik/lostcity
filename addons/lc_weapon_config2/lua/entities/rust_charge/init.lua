...
function ENT:Initialize()
	...
	if self.randomturnoff then
		self.Explode = CurTime() + math.Rand(6,12)
	end
...
function ENT:Think()
	...
	if CurTime() > self.NextBeep then
		if self.beep then
			self.NextBeep = CurTime() + 0.8
			self:EmitSound("darky_rust.c4-beep-loop")
		elseif self.randomturnoff then
	...
	if CurTime() > self.Explode then
		...
		if self.randomturnoff then
			if math.random(1,10)>=8 then 
				...
				self:EmitSound(self.sound.. math.random(1,4))
				self:StopSound("darky_rust.beancan-grenade-fuse")
...
function ENT:PhysicsCollide(data, phys)
	...
	timer.Simple(0, function()  -- to prevent "Changing collision rules within a callback is likely to cause crashes!" errors
		if not self:IsValid() then return end
		self:EmitSound("darky_rust.c4-stick")
