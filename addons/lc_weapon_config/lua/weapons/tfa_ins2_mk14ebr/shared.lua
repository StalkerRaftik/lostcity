...
SWEP.Manufacturer = "NSWCCD" --Gun Manufactrer (e.g. Hoeckler and Koch )
...
SWEP.PrintName				= "Mk.14 EBR [Fourth SR]"		-- Weapon name (Shown on HUD)
...
SWEP.Primary.Damage = 46 -- Damage, in standard damage points6
...
--Recoil Related
SWEP.Primary.KickUp = 0.87 -- This is the maximum upwards recoil (rise)
...
SWEP.Primary.StaticRecoilFactor = 1.33 --Amount of recoil to directly apply to EyeAngles.  Enter what fraction or percentage (in decimal form) you want.  This is also affected by a convar that defaults to 0.5.
--Firing Cone Related
...
SWEP.Primary.SpreadIncrement = .62 --What percentage of the modifier is added on, per shot.  Example val: 1/3.5
...
--Movespeed
SWEP.MoveSpeed = 0.775 --Multiply the player's movespeed by this.
SWEP.IronSightsMoveSpeed = SWEP.MoveSpeed  * 0.65 --Multiply the player's movespeed by this when sighting.
--[[PROJECTILES]]--
--[[VIEWMODEL]]--
SWEP.ViewModel			= "models/weapons/tfa_ins2/c_m14ebr.mdl" --Viewmodel path
SWEP.ViewModelFOV			= 69		-- This controls how big the viewmodel looks.  Less is more.
SWEP.ViewModelFlip			= false		-- Set this to true for CSS models, or false for everything else (with a righthanded viewmodel.)
SWEP.UseHands = true --Use gmod c_arms system.
SWEP.VMPos = Vector(-0.4, 0, -.1) --The viewmodel positional offset, constantly.  Subtract this from any other modifications to viewmodel position.
SWEP.VMAng = Vector(0, .02, 0) --The viewmodel angular offset, constantly.   Subtract this from any other modifications to viewmodel angle.
...
--[[WORLDMODEL]]--
SWEP.WorldModel			= "models/weapons/tfa_ins2/w_m14ebr.mdl" -- Weapon world model path
...
SWEP.Secondary.IronFOV = 66 -- How much you 'zoom' in. Less is more!  Don't have this be <= 0.  A good value for ironsights is like 70.
SWEP.IronSightsPos = Vector(-2.406, -2.85, 1.15)
SWEP.IronSightsAng = Vector(0, .02, 0)

SWEP.IronSightsPos_Kobra = Vector(-2.412, -9, .028)
SWEP.IronSightsAng_Kobra = Vector(0, 0, 0)

SWEP.IronSightsPos_EOTech = Vector(-2.41, -8, -.21)
SWEP.IronSightsAng_EOTech = Vector(0, 0, 0)

SWEP.IronSightsPos_RDS = Vector(-2.41, -9, -.275)
SWEP.IronSightsAng_RDS = Vector(0, 0, 0)

SWEP.IronSightsPos_2XRDS = Vector(-2.407, -9, -.28)
SWEP.IronSightsAng_2XRDS = Vector(0, 0, 0)

SWEP.IronSightsPos_C79 = Vector(-2.4095, -10, -.31)
SWEP.IronSightsAng_C79 = Vector(0, 0, 0)

SWEP.IronSightsPos_PO4X = Vector(-2.347, -9.5, .14)
SWEP.IronSightsAng_PO4X = Vector(0, 0, 0)

SWEP.IronSightsPos_Mosin = Vector(-2.387, -7.5, .133)
SWEP.IronSightsAng_Mosin = Vector(0, 0, 0)

SWEP.IronSightsPos_MX4 = Vector(-2.42, -7, -.095)
...
SWEP.SequenceLengthOverride = {[ACT_VM_RELOAD] = 3.35,
[ACT_VM_RELOAD_EMPTY] = 4.9,
[ACT_VM_DRAW] = .55,
[ACT_VM_DRAW_DEPLOYED] = 1.35,
[ACT_VM_DEPLOY_8] = 3.35,
[ACT_VM_DEPLOY_7] = 4.9,
[ACT_VM_FIREMODE] = .65,
[ACT_VM_IFIREMODE] = .65
 --Changes both the status delay and the nextprimaryfire of a given animation
}
SWEP.SequenceRateOverride = {[ACT_VM_RELOAD] = 1.3,
[ACT_VM_DRAW_DEPLOYED] = 1.05,
[ACT_VM_RELOAD_EMPTY] = 1.15,
[ACT_VM_DEPLOY_8] = 1.15,
[ACT_VM_DEPLOY_7] = 1.05, --Like above but changes animation length to a target
}
...
SWEP.VElements = {
	...
	["scope_mosin"] = { type = "Model", model = "models/weapons/tfa_ins2/upgrades/a_optic_mosin_l.mdl", bone = "A_Optic", rel = "", pos = Vector(0, 0, -1.5), angle = Angle(90, 90, 0), size = Vector(.85, .85, .85), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {}, active = false, bonemerge = false },
	...
	["suppressor"] = { type = "Model", model = "models/weapons/tfa_ins2/upgrades/a_suppressor_sec.mdl", bone = "A_Suppressor", rel = "", pos = Vector(0, 0, 1), angle = Angle(0, 0, 0), size = Vector(0.5, 0.5, 0.5), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} , bonemerge = true, active = false },
	["foregrip"] = { type = "Model", model = "models/weapons/tfa_ins2/upgrades/a_foregrip_sec.mdl", bone = "A_Foregrip", rel = "", pos = Vector(0, .1, 0.159), angle = Angle(0, -90, 0), size = Vector(0.435, 0.435, 0.435), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {}, bonemerge = false, active = false},
	["laser"] = { type = "Model", model = "models/weapons/tfa_ins2/upgrades/a_laser_rail.mdl", bone = "A_Underbarrel", rel = "", pos = Vector(-4, -0.87, .15), angle = Angle(0, 0, 0), size = Vector(.8, .8, .8), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {}, bonemerge = false, active = false},
	["laser_beam"] = { type = "Model", model = "models/tfa/lbeam.mdl", bone = "ValveBiped.Bip01_Spine4", rel = "laser", pos = Vector(1, 0, -.95), angle = Angle(0, 0, 0), size = Vector(3, 0.5, 0.5), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {}, bonemerge = false, active = false}
}
...
SWEP.WElements = {
	...
	["laser"] = { type = "Model", model = "models/weapons/tfa_ins2/upgrades/w_laser_sec.mdl", bone = "ATTACH_Laser", rel = "ref", pos = Vector(5, 0, 0), angle = Angle(0, 0, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {}, bonemerge = true, active = false },
} --Export from SWEP Creation Kit.  For each item that can/will be toggled, set active=false in its individual table

SWEP.LaserSightModAttachment = 6
...
SWEP.Attachments = {
	...
	[3] = { offset = { 0, 0 }, atts = { "ins2_fg_grip"}, order = 3 },
	[4] = { offset = { 0, 0 }, atts = { "ins2_ub_laser"}, order = 4},
	[5] = { offset = { 0, 0 }, atts = { "ins2_mag_ext_m14_30rd"}, order = 5 },
	[6] = { offset = { 0, 0 }, atts = { "ins2_mk14_camo"}, order = 6 },
	[7] = { offset = { 0, 0 }, atts = { "am_match", "am_magnum"}, order = 7 },
}
SWEP.AttachmentDependencies = {}--{["si_acog"] = {"bg_rail"}}
SWEP.AttachmentExclusions = {}--{ ["si_iron"] = {"bg_heatshield"} }

SWEP.StatusLengthOverride = {
	[ACT_VM_RELOAD] = 100 / 34.6,
	[ACT_VM_RELOAD_EMPTY] = 102 / 34.6,
	[ACT_VM_DEPLOY_8] = 90 / 34.6,
	[ACT_VM_DEPLOY_7] = 90 / 34.6,
}
...
SWEP.ViewModelBoneMods = {
	...
	["L Clavicle"] = { scale = Vector(.95, .95, .95), pos = Vector(0-.5, -.2, 1), angle = Angle(0, 0, 0)  },
	["A_Optic"] = { scale = Vector(.825, .825, .825), pos = Vector(0, -.1, 0), angle = Angle(0, 0, 0)  },
	["A_Suppressor"] = { scale = Vector(.925, .925, .925), pos = Vector(0, 0, 0), angle = Angle(0, 0, 0)  },
}
