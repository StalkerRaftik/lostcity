...
SWEP.Category				= "LostCity Weapon Other"                -- The category.  Please, just choose something generic or something I've already done if you plan on only doing like one swep..
SWEP.PrintName				= "AMP DSR-1 [Other Weapon]"	                  -- Weapon name (Shown on HUD)
SWEP.Manufacturer           = "DSR-precision GmbH"            -- Gun Manufactrer (e.g. Hoeckler and Koch )          
SWEP.Type                   = "Bolt-Action Sniper Rifle"
SWEP.Purpose				= "An Bullpup .338 Lapua Magnum Bolt Action Sniper Rifle."   
SWEP.Instructions		    = ""                          
...
SWEP.Primary.Sound                 = Sound("TFA_INS2.Warface_AMP_DSR1.Fire")            -- This is the sound of the weapon, when you shoot.
SWEP.Primary.SilencedSound         = Sound("TFA_INS2.Warface_AMP_DSR1.Fire_Suppressed") -- This is the sound of the weapon, when silenced.
...
SWEP.Primary.Velocity              = 1380                     -- Bullet Velocity in m/s
...
-- Recoil Related 
SWEP.Primary.KickUp              = 0.96                       -- This is the maximum upwards recoil (rise)
...
SWEP.Primary.StaticRecoilFactor  = 0.465                      -- Amount of recoil to directly apply to EyeAngles.  Enter what fraction or percentage (in decimal form) you want.  This is also affected by a convar that defaults to 0.5.

-- Firing Cone Related
SWEP.Primary.Spread              = .0228                      -- This is hip-fire acuracy.  Less is more (1 is horribly awful, .0001 is close to perfect)
SWEP.Primary.IronAccuracy        = .000032                    -- Ironsight accuracy, should be the same for shotguns

-- Unless you can do this manually, autodetect it.  If you decide to manually do these, uncomment this block and remove this line.
SWEP.Primary.SpreadMultiplierMax = 5.33                       -- How far the spread can expand when you shoot. Example val: 2.5
SWEP.Primary.SpreadIncrement     = 5                          -- What percentage of the modifier is added on, per shot.  Example val: 1/3.5
SWEP.Primary.SpreadRecovery      = 2.8                        -- How much the spread recovers, per second. Example val: 3

-- Range Related
SWEP.Primary.Range               = 1.75 * (3280.84 * 16)      -- The distance the bullet can travel in source units.  Set to -1 to autodetect based on damage/rpm.
...
SWEP.Primary.RangeFalloffLUT = {
	...
	lut = {
		...
		{range = 1000, damage = 1},
		{range = 1500, damage = 1},
		{range = 1750, damage = 0.965},
		{range = 1750, damage = 0.965},
		{range = 1750, damage = 0.965},
		{range = 2500, damage = 0.59},
	}
}
...
-- Misc
SWEP.IronRecoilMultiplier        = 0.92                       -- Multiply recoil by this factor when we're in ironsights.  This is proportional, not inversely.
SWEP.CrouchAccuracyMultiplier    = 0.62                       -- Less is more.  Accuracy * 0.5 = Twice as accurate, Accuracy * 0.1 = Ten times as accurate

-- Movespeed
SWEP.MoveSpeed                   = 0.75                       -- Multiply the player's movespeed by this.
SWEP.IronSightsMoveSpeed         = 0.68                       -- Multiply the player's movespeed by this when sighting.
...
SWEP.ViewModel			   = "models/weapons/c_ins2_warface_amp_dsr1.mdl" 
SWEP.ViewModelFOV	       = 70		                          -- This controls how big the viewmodel looks.  Less is more.
SWEP.ViewModelFlip	       = false		                      -- Set this to true for CSS models, or false for everything else (with a righthanded viewmodel.)
SWEP.UseHands              = true                             -- Use gmod c_arms system.

SWEP.VMPos                 = Vector(-0.2, -0.6, -0.3)         -- The viewmodel positional offset, constantly.  Subtract this from any other modifications to viewmodel position.
...
SWEP.SequenceLengthOverride = {                               -- Changes both the status delay and the nextprimaryfire of a given animation
	[ACT_VM_PULLBACK_LOW]   = 34 / 30,
	[ACT_VM_PULLBACK_HIGH]  = 34 / 30,
}   
...
SWEP.EventTable = {
	[ACT_VM_PULLBACK_LOW] = {
		{ ["time"] = 16.5 / 39, ["type"] = "lua", ["value"] = function(wep,vm)
	...
	[ACT_VM_PULLBACK_HIGH] = {
		{ ["time"] = 21 / 39, ["type"] = "lua", ["value"] = function(wep,vm)
			if wep and wep.EventShell then
				wep:EventShell()
			end
		end, ["client"] = true, ["server"] = true }
	...
	[ACT_VM_RELOAD_EMPTY] = {
		{ ["time"] = 22.35 / 30, ["type"] = "lua", ["value"] = function(wep,vm)
			if wep and wep.EventShell then
				wep:EventShell()
			end
		end, ["client"] = true, ["server"] = true }
...
SWEP.WorldModel	   = "models/weapons/w_ins2_warface_amp_dsr1.mdl" -- Weapon world model path
...
SWEP.IronSightsPos     = Vector(-2.42, -1, 0.12)
...
SWEP.IronSightsPos_Kobra     = Vector(-2.435, -1, 0.125)
SWEP.IronSightsAng_Kobra     = Vector(0.1, 0.005, 0)

SWEP.IronSightsPos_EOTech    =  Vector(-2.43, -1, -0.035)
SWEP.IronSightsAng_EOTech    = Vector(0, -0.01, 0)

SWEP.IronSightsPos_RDS       = Vector(-2.435, -1, 0.05)  
SWEP.IronSightsAng_RDS       = Vector(0, -0.015, 0)

SWEP.IronSightsPos_2XRDS     = Vector(-2.42, -2.42, 0.18)
SWEP.IronSightsAng_2XRDS     = Vector(0, 0, 0)
SWEP.Secondary.IronFOV_2XRDS = 60

SWEP.IronSightsPos_Mosin     = Vector(-2.405, -1.3, 0.62)
SWEP.IronSightsAng_Mosin     = Vector(0, -0.02, 0)

SWEP.IronSightsPos_C79       = Vector(-2.42, -2, -0.145)
SWEP.IronSightsAng_C79       = Vector(0, -0.004, 0)
 
SWEP.IronSightsPos_PO4X      = Vector(-2.365, -1.25, 0.272)
SWEP.IronSightsAng_PO4X      = Vector(0, -0.01, 0)
SWEP.Secondary.IronFOV_PO4X  = 70

SWEP.IronSightsPos_MX4       = Vector(-2.435, -1, 0.062)
...
SWEP.InspectPos   = Vector(8, -5, -2.241) 
...
SWEP.Attachments = {
	...
	[6] = { atts = { "warface_dsr_camo04_skin" } },
}
...
SWEP.ViewModelBoneMods = {
	["A_Optic"] = { scale = Vector(0.75, 0.75, 0.75), pos = Vector(0, 0, 0), angle = Angle(0, 0, 0) },
	["A_Suppressor"] = { scale = Vector(0.9, 1, 0.9), pos = Vector(0, 0, 1.3), angle = Angle(0, 0, 0) },
	["A_LaserFlashlight"] = { scale = Vector(0.9, 0.9, 0.9), pos = Vector(0, 0, 0.5), angle = Angle(0, 0, 0) }, 
	...
	["L Finger3"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, -5, 0) }, 
	["R Finger0"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, 0, 0) }, 
	["R Finger4"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, 6.5, 0) }, 
	["R Finger41"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, 8, 0) }, 
	["R Finger42"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, 5, 0) }, 
}	

SWEP.WorldModelBoneMods = {
	["ATTACH_ModKit"] = { scale = Vector(0.8, 0.8, 0.8), pos = Vector(0, 0, -0.2), angle = Angle(0, 0, 0) },
	["ATTACH_Eject"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, 0, 0) },
	["ATTACH_Laser"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0.2, 0.2), angle = Angle(0, 0, 0) },
}

SWEP.VElements = {
	["sights_folded"] = { type = "Model", model = "models/weapons/upgrades/a_standard_dsr1.mdl", bone = "A_Optic", rel = "", pos = Vector(-2.62, 1.46, -12.5), angle = Angle(90, 90, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {}, bonemerge = true, active = true},
...
SWEP.WElements = {
	...
	["sights_folded"] = { type = "Model", model = "models/weapons/upgrades/w_standard_dsr1.mdl", bone = "ATTACH_ModKit", rel = "ref", pos = Vector(0, 0, 0), angle = Angle(0, 0, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {}, bonemerge = true, active = true },
