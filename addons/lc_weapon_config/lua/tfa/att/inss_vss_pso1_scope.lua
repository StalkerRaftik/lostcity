...
ATTACHMENT.Name        = "VSS PSO Scope"
ATTACHMENT.ShortName   = "PSO"

ATTACHMENT.Icon        = "entities/inss_vss_pso1_scope.png" -- Revers to label, please give it an icon though!  This should be the path to a png, like "entities/tfa_ammo_match.png"

ATTACHMENT.Description = { 
TFA.AttachmentColors["+"], "7x Zoom", "Looks Cool", "Is Politically Correct",  
TFA.AttachmentColors["-"], "10% Slower Aimed Walking" 
}
...
ATTACHMENT.WeaponTable = {
	...
	["IronSightsPos"] = function( wep, val ) return wep.IronSightsPos_vss_pso or val end,
	["IronSightsAng"] = function( wep, val ) return wep.IronSightsAng_vss_pso or val end,
	...
	["Secondary"] = {
		["IronFOV"] = function( wep, val ) return wep.Secondary.IronFOV_vss_pso or val * 0.9 end,
	...
	["RTScopeFOV"] = function(wep, val) return (wep.RTScopeFOV_vss_pso or wep:GetStat("Secondary.IronFOV")) / wep:GetStat("Secondary.ScopeZoom") / 2 end,
	["RTOpaque"] = -1,
...
function ATTACHMENT:Attach(wep)
	...
	wep.RTCode = function( myself , rt, scrw, scrh)
		...
		local att, ts
		if wep:VMIV() then
			att = wep.OwnerViewModel:GetAttachment( wep.RTAttachment_vss_pso or 0 )
		end
		...
		if wep.ScopeAngleTransforms_vss_pso then
			ang:RotateAroundAxis(ang:Right(), wep.ScopeAngleTransforms_vss_pso.p )
			ang:RotateAroundAxis(ang:Up(), wep.ScopeAngleTransforms_vss_pso.y )
			ang:RotateAroundAxis(ang:Forward(), wep.ScopeAngleTransforms_vss_pso.r )
		end
		cd.angles = ang
		...
		if ts then
			...
			if wep.ScopeOverlayTransforms_vss_pso then
				scrpos.x = scrpos.x + wep.ScopeOverlayTransforms_vss_pso[1]
				scrpos.y = scrpos.y + wep.ScopeOverlayTransforms_vss_pso[2]
			end
			...
			if wep.ScopeOverlayTransformMultiplier_vss_pso then
				scrpos.x = scrpos.x * wep.ScopeOverlayTransformMultiplier_vss_pso
				scrpos.y = scrpos.y * wep.ScopeOverlayTransformMultiplier_vss_pso
			end
			...
			local rtow, rtoh = 0, 0
			if wep.RTScopeOffset_vss_pso then
				rtow = self.RTScopeOffset_vss_pso[1] * rttw
				rtoh = self.RTScopeOffset_vss_pso[2] * rtth
			end
			local rtw, rth = rttw * 1, rtth * 1
			if self.RTScopeScale_vss_pso then
				rtw = rtw * self.RTScopeScale_vss_pso[1]
				rth = rth * self.RTScopeScale_vss_pso[2]
			end
			local distfac = math.pow( 1 - math.Clamp( ( att.Pos:Distance( wep.Owner:GetShootPos() ) - ( wep.ScopeDistanceMin_vss_pso or 4 ) ) / ( wep.ScopeDistanceRange_vss_pso or 12 ), 0, 1 ), 1 )
