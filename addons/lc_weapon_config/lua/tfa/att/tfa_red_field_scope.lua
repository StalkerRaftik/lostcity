...
ATTACHMENT.Name = "Redfield Scope"
...
ATTACHMENT.Icon = "entities/redfield_scope.png" --Revers to label, please give it an icon though!  This should be the path to a png, like "entities/tfa_ammo_match.png"
ATTACHMENT.ShortName = "Red Scope"
...
ATTACHMENT.WeaponTable = {
	...
	["IronSightsPos"] = function( wep, val ) return wep.IronSightsPos_Red_Field_Scope or val end,
	["IronSightsAng"] = function( wep, val ) return wep.IronSightsAng_Red_Field_Scope or val end,
	["IronSightsSensitivity"] = function(wep,val)
		local res = val * wep:Get3DSensitivity( )
		return res, false, true
	end ,
	["Secondary"] = {
		["IronFOV"] = function( wep, val ) return wep.Secondary_Red_Field_Scope or val * 0.9 end,
	...
	["RTScopeFOV"] = function(wep, val) return (wep.RTScopeFOV_Red_Field_Scope or wep:GetStat("Secondary.IronFOV")) / wep:GetStat("Secondary.ScopeZoom") / 2 end,
	["RTOpaque"] = -1,
...
function ATTACHMENT:Attach(wep)
	...
	wep.RTCode = function( myself , rt, scrw, scrh)
		...
		local att, ts
		if wep:VMIV() then
			att = wep.OwnerViewModel:GetAttachment( wep.RTAttachment_Red_Field_Scope or 0 )
		end
		...
		if wep.ScopeAngleTransforms_SMLE_OPTIC then
			ang:RotateAroundAxis(ang:Right(), wep.ScopeAngleTransforms_Red_Field_Scope.p )
			ang:RotateAroundAxis(ang:Up(), wep.ScopeAngleTransforms_Red_Field_Scope.y )
			ang:RotateAroundAxis(ang:Forward(), wep.ScopeAngleTransforms_Red_Field_Scope.r )
		end
		cd.angles = ang
		...
		if ts then
			...
			if wep.ScopeOverlayTransforms_SMLE_OPTIC then
				scrpos.x = scrpos.x + wep.ScopeOverlayTransforms_Red_Field_Scope[1]
				scrpos.y = scrpos.y + wep.ScopeOverlayTransforms_Red_Field_Scope[2]
			end
			...
			if wep.ScopeOverlayTransformMultiplier_SMLE_OPTIC then
				scrpos.x = scrpos.x * wep.ScopeOverlayTransformMultiplier_Red_Field_Scope
				scrpos.y = scrpos.y * wep.ScopeOverlayTransformMultiplier_Red_Field_Scope
			end
			...
			local rtow, rtoh = 0, 0
			if wep.RTScopeOffset_SMLE_OPTIC then
				rtow = self.RTScopeOffset_Red_Field_Scope[1] * rttw
				rtoh = self.RTScopeOffset_Red_Field_Scope[2] * rtth
			end
			local rtw, rth = rttw * 1, rtth * 1
			if self.RTScopeScale_SMLE_OPTIC then
				rtw = rtw * self.RTScopeScale_Red_Field_Scope[1]
				rth = rth * self.RTScopeScale_Red_Field_Scope[2]
			end
			local distfac = math.pow( 1 - math.Clamp( ( att.Pos:Distance( wep.Owner:GetShootPos() ) - ( wep.ScopeDistanceMin_Red_Field_Scope or 3 ) ) / ( wep.ScopeDistanceRange_Red_Field_Scope or 10 ), 0, 1 ), 1 )
