...
function ENT:SimulateVehicle( curtime )
	...
	if Active then
		...
		self:SimulateTransmission(W,S,0,Alt,Space,GearUp,GearDown,transmode,IdleRPM,Powerbandstart,Powerbandend,sportsmode,cruise,curtime)
...
function ENT:TrunkLock()
	self.TrunkLocked = true
end

function ENT:TrunkUnLock()
	self.TrunkLocked = false
end
...
function ENT:Use( ply )
	local vPosition = Vector( self:OBBCenter()[1], self:OBBMins()[2], 10 )
	vPosition = self:LocalToWorld( vPosition)
	if (ply:GetPos():DistToSqr(vPosition) <= 2000) then
		if (ply.lastTrunk or 0) < CurTime() and self.IsStorage then
			if self.TrunkLocked then return end
			-- if self:GetNVar("TrunkLocked") == true then return end
		...
		ply:SetActiveWeapon(ply:GetWeapon("weapon_rphands"))
...
function ENT:SetPassenger( ply )
	...
	if self.Locked or self:HasPassengerEnemyTeam( ply ) then
