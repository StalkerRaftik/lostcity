...
local sizex = screenw * (Widescreen and 0.8 or 1.32)
local sizey = screenh
local xpos = sizex * 0.020
local ypos = sizey * 0.75
local x = xpos * (Widescreen and 56 or 32)
local y = ypos * 1.055
...
local function drawsimfphysHUD(vehicle,SeatCount)
	...
	if AltHud and not ForceSimpleHud and not SimpleHudIsForced then
		...
		surface.DrawTexturedRect( x + radius * 1.15 + o_x, y - radius * -0.9 + o_y, sizex * 0.014, sizex * 0.014 )
		
		local mat = FogLightsOn and fog_on or fog_off
		surface.SetMaterial( mat )
		surface.DrawTexturedRect( x + radius * 1.12 + o_x, y - radius * -0.6 + o_y, sizex * 0.018, sizex * 0.018 )
		
		local mat = cruisecontrol and cruise_on or cruise_off
		surface.SetMaterial( mat )
		surface.DrawTexturedRect( x + radius * 1.11 + o_x, y - radius * -0.3 + o_y, sizex * 0.02, sizex * 0.02 )
		
		local mat = HandBrakeOn and hbrake_on or hbrake_off
		surface.SetMaterial( mat )
		surface.DrawTexturedRect( x + radius * 1.13 + o_x, y - radius * -0.05 + o_y, sizex * 0.018, sizex * 0.018 )
		...
		local print_text = Hudmph and "MPH" or "КМ/Ч"
		...
		draw.SimpleText( digit_1, "simfphysfont2", x + radius * 0.86 + o_x, y + radius * 0.65 + o_y, col1, 1, 1 )
		draw.SimpleText( digit_2/ 10, "simfphysfont2", x + radius * 0.60 + o_x, y + radius * 0.65 + o_y, col2, 1, 1 )
		draw.SimpleText( digit_3 / 100, "simfphysfont2", x + radius * 0.34 + o_x, y + radius * 0.65 +  o_y, col3, 1, 1 )
