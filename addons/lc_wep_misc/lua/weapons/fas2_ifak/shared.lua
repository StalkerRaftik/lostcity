...
if CLIENT then
    SWEP.PrintName = "Универсальная аптечка первой помощи"
    SWEP.Slot = 0
    SWEP.SlotPos = 0
...
function SWEP:Equip(own)
	own:GiveAmmo(0, "Bandages")
	own:GiveAmmo(0, "Quikclots")
	own:GiveAmmo(0, "Hemostats")
end
...
if CLIENT then
	...
	function SWEP:Draw3D2DCamera()
		...
		cam.Start3D2D(pos + ang:Right() * -3.8 - ang:Forward() * 3.5, ang, 0.015 * GetConVarNumber("fas2_textsize"))
			am = self.Owner:GetAmmoCount("Bandages")
			draw.ShadowText("Бинты: " .. am, "FAS2_HUD24", -40, 0, White, Black, 2, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
			
			if am > 0 then
				draw.ShadowText("ЛКМ - использовать бинт", "FAS2_HUD24", -40, -40, Green, Black, 2, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
				draw.ShadowText("Слабое лечение", "FAS2_HUD24", -40, -60, Green, Black, 2, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
			end
		cam.End3D2D()
		...
		cam.Start3D2D(pos + ang:Right() * -3.5 + ang:Forward(), ang, 0.015 * GetConVarNumber("fas2_textsize"))
			...
			if am > 0 then
				draw.ShadowText("Гемостат: " .. am, "FAS2_HUD24", -20, 0, White, Black, 2, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
				draw.ShadowText("ПКМ - использовать гемостат", "FAS2_HUD24", -20, -40, Green, Black, 2, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
				draw.ShadowText("Сильное лечение", "FAS2_HUD24", -20, -60, Green, Black, 2, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
			else
				am = self.Owner:GetAmmoCount("Quikclots")
				
				draw.ShadowText("Марля: " .. am, "FAS2_HUD24", -20, 0, White, Black, 2, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
				
				if am > 0 then
					draw.ShadowText("ПКМ - спользовать марлю", "FAS2_HUD24", -20, -40, Green, Black, 2, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
					draw.ShadowText("Среднее лечение", "FAS2_HUD24", -20, -60, Green, Black, 2, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
				end
			end
		cam.End3D2D()
	end
	...
	function SWEP:DrawHUD()
		...
		if self.Target then
			draw.ShadowText("Лечить: " .. (self.Target != self.Owner and "человека напротив" or "себя"), "FAS2_HUD24", x / 2, y / 2 + 200, Color(255, 255, 255, self.CrossAlpha), Color(0, 0, 0, self.CrossAlpha), 2, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		else
			draw.ShadowText("Лечить: себя", "FAS2_HUD24", x / 2, y / 2 + 200, Color(255, 255, 255, self.CrossAlpha), Color(0, 0, 0, self.CrossAlpha), 2, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		end
		
		if GetConVarNumber("fas2_customhud") <= 0 or self.Owner:ShouldDrawLocalPlayer() then
			am = self.Owner:GetAmmoCount("Bandages")
			
			draw.ShadowText("Бинты: " .. am, "FAS2_HUD36", x2 - 100, y2 + 125, White, Black, 2, TEXT_ALIGN_RIGHT, TEXT_ALIGN_LEFT)
			
			if am > 0 then
				draw.ShadowText("ЛКМ - использовать бинт", "FAS2_HUD24", x2 - 100, y2 + 100, Green, Black, 2, TEXT_ALIGN_RIGHT, TEXT_ALIGN_LEFT)
				draw.ShadowText("Слабое лечение", "FAS2_HUD24", x2 - 100, y2 + 75, Green, Black, 2, TEXT_ALIGN_RIGHT, TEXT_ALIGN_LEFT)
			end
			
			am = self.Owner:GetAmmoCount("Hemostats")
			
			if am > 0 then
				draw.ShadowText("Гемостат: " .. am, "FAS2_HUD36", x2 + 100, y2 + 125, White, Black, 2, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
				draw.ShadowText("ПКМ - использовать гемостат", "FAS2_HUD24", x2 + 100, y2 + 100, Green, Black, 2, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
				draw.ShadowText("Сильное лечение", "FAS2_HUD24", x2 + 100, y2 + 75, Green, Black, 2, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
			else
				am = self.Owner:GetAmmoCount("Quikclots")
				
				draw.ShadowText("Марля: " .. am, "FAS2_HUD36", x2 + 100, y2 + 125, White, Black, 2, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
				
				if am > 0 then
					draw.ShadowText("ПКМ - использовать марлю", "FAS2_HUD24", x2 + 100, y2 + 100, Green, Black, 2, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
					draw.ShadowText("Среднее лечение", "FAS2_HUD24", x2 + 100, y2 + 75, Green, Black, 2, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
				end
			end
		end
	end
