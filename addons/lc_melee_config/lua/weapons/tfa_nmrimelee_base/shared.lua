...
function SWEP:Think2()
	...
	if stat == TFA.GetStatus("NMRIH_MELEE_CHARGE_LOOP") and not self.Owner:KeyDown(IN_ATTACK) then
		...
		timer.Simple(self.OwnerViewModel:SequenceDuration()-0.3, function()
			self.charge = false
		end)
...
function SWEP:Reload(released, ovr, ...)
	if not self:VMIV() or self.charge == true then return end
	if self.BlockTime > CurTime() then return end
	self.BlockTime = CurTime() + 2

	self.OnBlock = true
	if SERVER then
		self.Owner:EmitSound( "npc/stalker/stalker_footstep_right1.wav" )
	end

	self:ScheduleStatus(TFA.Enum.STATUS_SHOOTING, 1)
	if SERVER then
	...
	if CLIENT then
		local StdAng = self.VMAng
		local NewAng = Vector(0, 10, -50)
		local inAnim = true
		hook.Add("Think", "BlockSmoothAnim", function()
			if inAnim then
				if not self.VMAng then return end
				self.VMAng = Vector( 
					Lerp( 0.1, self.VMAng[1], StdAng[1] + NewAng[1] ), 
					Lerp( 0.1, self.VMAng[2], StdAng[2] + NewAng[2]), 
					Lerp( 0.1, self.VMAng[3], StdAng[3] + NewAng[3]) 
				)
			else
				if not self.VMAng then return end
				self.VMAng = Vector( 
					Lerp( 0.1, self.VMAng[1], StdAng[1]), 
					Lerp( 0.1, self.VMAng[2], StdAng[2]), 
					Lerp( 0.1, self.VMAng[3], StdAng[3]) 
				)
			end
		end)
		...
		timer.Simple(1, function()
			if self.VMAng then self.VMAng = StdAng end
			hook.Remove( "Think", "BlockSmoothAnim")
		end)
	end
