...
function EFFECT:Init(data)
	...
	local rollparticle=emitter:Add("sprites/mat_jack_job_basicglow",vOffset)
	if(rollparticle)then
		...
		rollparticle:SetColor(175,100,255)
		rollparticle:SetStartAlpha(255)
		rollparticle:SetEndAlpha(0)
		rollparticle:SetStartSize(160*self.Scayul)
	...
	local rollparticle2=emitter:Add("sprites/mat_jack_job_basicglow",vOffset)
	...
	if(dlight)then
		...
		dlight.g=100
		dlight.b=255
		dlight.Brightness=20*self.Scayul
		dlight.Size=1000*self.Scayul
		dlight.Decay=350*self.Scayul
		dlight.DieTime=CurTime()+1*self.Scayul
		dlight.Style=0
	end
end
/*---------------------------------------------------------
