...
local curTime = CurTime(); 
	if ((data.DeltaTime > 0) and (data.HitEntity:GetClass() == "acetone") and (self:GetNWInt("macid")<1) and (self:GetNWString("status") != "ready")) then
	...
	if ((data.DeltaTime > 0) and (data.HitEntity:GetClass() == "benznatriya") and (self:GetNWInt("sulfur")<1) and (self:GetNWString("status") != "ready")) then
		if (data.HitEntity:GetNWInt("amount")>0) then
			data.HitEntity:SetNWInt("amount", math.Clamp(data.HitEntity:GetNWInt("amount") - 1, 0, 100));
			if EML_Pot_DestroyEmpty then
				if (data.HitEntity:GetNWInt("amount")==0) then	
