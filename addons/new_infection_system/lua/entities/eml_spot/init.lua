...
local curTime = CurTime(); 		
	if ((data.DeltaTime > 0) and (data.HitEntity:GetClass() == "pervodoroda") and (self:GetNWInt("redp")<1) and (self:GetNWString("status") != "ready")) then	
	...
	if ((data.DeltaTime > 0) and (data.HitEntity:GetClass() == "propglicogol") and (self:GetNWInt("ciodine")<1) and (self:GetNWString("status") != "ready")) then		
		if (data.HitEntity:GetNWInt("amount")>0) then
			data.HitEntity:SetNWInt("amount", math.Clamp(data.HitEntity:GetNWInt("amount") - 1, 0, 100));
			if (data.HitEntity:GetNWInt("amount")==0) then	
				data.HitEntity:VisualEffect();
...
local curTime = CurTime();
	if (!self.nextUse or curTime >= self.nextUse) then
		...
		elseif ((self:GetNWString("status")=="ready") and ((self:GetNWInt("redp")>0) and (self:GetNWInt("ciodine")>0))) then			
			...
			meth = ents.Create("zombification_cure");
