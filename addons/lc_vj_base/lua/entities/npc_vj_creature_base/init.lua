...
ENT.SightDistance = 1200 -- How far it can see
...
ENT.DamageByPlayerDispositionLevel = 2 -- 0 = Run it every time | 1 = Run it only when friendly to player | 2 = Run it only when enemy to player
...
function ENT:Think()
	...
	if self.Dead == false && self.HasBreathSound == true && self.HasSounds == true then
		if CurTime() > self.NextBreathSoundT then
			...
			self.NextBreathSoundT = CurTime() + 3
		end
	end
	--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--
	if GetConVarNumber("ai_disabled") == 0 then
		...
		*/
		...
		if IsValid(ene) then
			...
			-- if ene:Crouching() == true then
			-- 	if self:GetPos():DistToSqr(ene:GetPos()) < 220*220 then 
					...
					if (v:IsNPC() && (!VJ_IsHugeMonster)) or v:IsPlayer() then
						if self.MeleeAttackBleedEnemy == true then
							...
							if math.random(1,self.MeleeAttackBleedEnemyChance) == 1 then
								timer.Create("timer_melee_bleedply",self.MeleeAttackBleedEnemyTime,self.MeleeAttackBleedEnemyReps,function() if IsValid(v) then v:TakeDamage(1,self,self) end end)
							end
			...
			if ((self.Behavior == VJ_BEHAVIOR_NEUTRAL && self.Alerted == true) or self.Behavior != VJ_BEHAVIOR_NEUTRAL) && self.DisableFindEnemy == false then
				if (seethroughwall == true) or (self:Visible(v) && (vDistanceToMy < sightdist)) then
					if (self.FindEnemy_UseSphere == false && radiusoverride == 0 && (self:GetForward():Dot((vPos - MyPos):GetNormalized()) > math.cos(math.rad(self.SightAngle)))) or (self.FindEnemy_UseSphere == true or radiusoverride == 1) then
						if self:CustomFindCheck(v, vDistanceToMy) ~= false then
...
function ENT:CreateGibEntity(Ent,Models,Tbl_Features,CustomCode)
	...
	cleanup.ReplaceEntity(gib)
		if gib:GetClass() == "prop_ragdoll" then gib:Fire("FadeAndRemove","",30) end
		-- if gib:GetClass() == "prop_physics" then print('2') gib:Fire("kill","",30) end
...
function ENT:CreateDeathCorpse(dmginfo,hitgroup)
	--_BeforeCorpseSpawned(dmginfo,hitgroup)
