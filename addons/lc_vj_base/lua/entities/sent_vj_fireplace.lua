...
function ENT:Think()
	...
	for k, v in pairs(ents.FindInSphere( self:GetPos(), 200 )) do
		local distSqr = self:GetPos():DistToSqr(v:GetPos())
		if fuelTbl[v:GetClass()] and distSqr < 50*50 then
			fuel = v
		end
		if v:IsPlayer() and self:GetNWBool("VJ_FirePlace_Activated") == true then
			if distSqr < 200*200 then
				v:SetNVar("FireAround", CurTime()+3, NETWORK_PROTOCOL_PRIVATE)
			end
			if distSqr <= 30*30 then
				v:Ignite(math.Rand(3,5))
			end
		end
...
function ENT:Touch(entity)
	print(entity:GetPos():Distance(self:GetPos()))
