...
if SERVER then
	...
	function StormFox.SetWeather( sWeatherId, flMagnitude, tTime )
		if rp and rp.CurEvent then return end
		if sWeatherId == "clear" then sWeatherId = "lc_clear" end

		if not StormFox.GetWeatherType( sWeatherId ) and sWeatherId ~= "lc_clear" then print( "[StormFox] Weather not found:", sWeatherId ) print(debug.traceback()) return end
		local old_w = StormFox.GetNetworkData("Weather","lc_clear")
		...
		if flMagnitude <= 0 then
			hook.Call("StormFox - NewWeather",nil,"lc_clear",old_w)
		else
			hook.Call("StormFox - NewWeather",nil,sWeatherId,old_w)
		end
	end
	StormFox.SetWeather("lc_clear",0.8)
end
...
hook.Add("StormFox - NetDataChange","StormFox - WeatherLogic",function(var,weather)
	...
	local sWeatherID = StormFox.GetNetworkData( "Weather", "lc_clear" )
...
local function weatherThink()
	...
	-- Load data
		...
		if flMagnitude <= 0 then
			StormFox.SetNetworkData( "Weather", "lc_clear" )
			sWeatherID = "lc_clear"
		end
		if sWeatherID == "lc_clear" then
			flMagnitude = 1
		end
		...
		-- Update timevars
			for key,_ in pairs(StormFox.Weather.TimeDependentData) do
				...
				elseif flMagnitude > 0 then
					local base = StormFox.GetWeatherType("lc_clear"):GetData(key,flTime)
