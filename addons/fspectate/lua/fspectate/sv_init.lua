...
local function startSpectating(ply, target)
    ...
    local targetText = IsValid(target) and target:IsPlayer() and (target:Nick() .. " (" .. target:SteamID() .. ")") or IsValid(target) and "за энтити/NPC" or ""
    ply:ChatPrint("Вы наблюдаете " .. targetText)
end

local function Spectate(ply, cmd, args)
    CAMI.PlayerHasAccess(ply, "FSpectate", function(b, _)
        if not b then ply:ChatPrint("Нет доступа!") return end
        if not ba.canAdmin(ply) then ply:ChatPrint("Начните администрировать!") return end

        local target = findPlayer(args[1])
        if target == ply then ply:ChatPrint("Не могу найти цель.") return end
...
local function playerVoice(listener, talker)
    ...
    if not IsValid(listener.FSpectatingEnt) or not listener.FSpectatingEnt:IsPlayer() then
        ...
        if not DarkRP or not spectatePos then return end
